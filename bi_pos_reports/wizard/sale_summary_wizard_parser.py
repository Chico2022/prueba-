...
class SaleSummaryReport(models.AbstractModel):
	...
	def _get_report_values(self, docids, data=None):
		...
		datetime_object = datetime.strptime(str(sale_summary_rec.start_dt)+ " 00:00:00", '%Y-%m-%d %H:%M:%S')
		date_start = tz_session.localize(fields.Datetime.from_string(datetime_object)).astimezone(pytz.utc)
		datetime_object = datetime.strptime(str(sale_summary_rec.end_dt)+ " 23:59:59", '%Y-%m-%d %H:%M:%S')
		date_end = tz_session.localize(fields.Datetime.from_string(datetime_object)).astimezone(pytz.utc)
